...
class PikaTransport(CommonTransport):
    ...
    def _subscribe_broadcast(
        ...
        **kwargs,
        ...
        # Is locking here necessary? PikaThread manages threadsafe here,
        # but I guess we could be managing threadstate elsewhere in this
        # class? Still not confident that this class is threadsafe in
        # the general sense
        with self._lock:
            if not self._pika_thread.alive:
                assert False
                self.disconnect()  # ERROR: self.disconnect() also locks self._lock
                raise workflows.Disconnected("No connection to RabbitMQ server.")
                # WIP notes:
                # - not sure prefetch_count makes much sense for single-consumer
                #   queues attached to a fanout exchange
                # - What does non-auto-ack even mean in this case?
                # - Exclusive connection prohibits reconnection
